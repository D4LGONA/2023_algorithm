...
V = [[-1 for _ in range(num_vertex)] for _ in range(num_vertex) ] # 경유지를 저장하는 용도, -1은 직접 간다
for s, e, w in edges:
  D[s][e] = w
  D[e][s] = w * 6 // 10

for k in range(num_vertex): # 경유지
  for s in range(num_vertex): # 출발지
    if s == k: continue
    for e in range(num_vertex): # 도착지
      if e == k or e == s: continue
      if D[s][e] > D[s][k] + D[k][e]: # s에서 e 가는데 k 들러 가는게 더 가까우면
        D[s][e] = D[s][k] + D[k][e] # 그렇게 갈게요
        V[s][e] = k # sㅇㅔ서 e 갈때는 k를 들렸다 가야 한다

def getPath(s, e):
  k = V[s][e]
  if k == -1: return f'=>{e}' # 직접 가는 길
...
for s in range(num_vertex):
  for e in range(num_vertex):
    if s == 2: continue
    path = getPath(s, e)
    cost = D[s][e] # D에 있는 갓ㅂ을 활용 s 와 e 활용 ... 
    print(f'{s}{path} ({cost})')
