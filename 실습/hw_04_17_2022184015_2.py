...
covered_edge = 0
while covered_edge < len(edges):
  u = list(vertices).pop(randrange(len(vertices)))
  if not g[u]: continue
  ...
  for n in (u, v):
    ...
    for k in range(num_vertex):
      if k in g[n]: # n에서 k로 가는 선이 살아있으면
        g[n].remove(k)  # 지운다
        if n in g[k]: # k에서 n으로 가는 선이 살아있따면
          g[k].remove(n) # 지운다
        covered_edge += 1 # 지운 선의 개수를 하나 증가?
